...
class UMGLogger:
    ...
    def __init__(self, name, level=logging.INFO):
        ...
        with UMGLogger._lock:
            if UMGLogger._handler is None:
                self.create_file_handler()
            self.logger.addHandler(UMGLogger._handler)
    ...
    def create_file_handler(self):
        ...
        filename = f"run-{now}-log.txt"

        UMGLogger._filename = filename
        handler = logging.FileHandler(filename)
        formatter = logging.Formatter("%(name)s => %(message)s")
        handler.setFormatter(formatter)
        UMGLogger._handler = handler
...
# # Example usage
# logger1 = UMGLogger("MainModule")
# logger2 = UMGLogger("AnotherModule")  # Both use the same file
